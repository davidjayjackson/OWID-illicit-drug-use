...
od_longer %>% filter(name !="Total") %>% ggplot() + geom_line(aes(x=Year,y=value,col=name)) + scale_y_log10(labels=comma) +
  labs(title="Worldwide Overdoes Deaths by Drug and Year",y="Deaths Do TO Overdose")
```

```{r}
od_longer %>% filter(name !="Total") %>% ggplot() + geom_line(aes(x=Year,y=value/sum(value),col=name)) +
  labs(title="Worldwide Overdoes Deaths by Drug and Year",y=" % of Deaths Do TO Overdose") + scale_y_continuous(labels=percent)
```